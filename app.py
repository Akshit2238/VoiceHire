...
def get_available_slots():
    ...
    try:
        ...
        result = [
            {"slot": s, "available": s not in booked_slots}
            for s in TIME_SLOTS
        ]
...
def get_my_bookings():
    ...
    try:
        ...
        # Enrich with worker/customer names
...
def checkin_booking(booking_id):
    ...
    try:
        ...
        # Only the customer or worker on this booking can check in
        if booking['customer_id'] != uid and booking['worker_id'] != uid:
            return jsonify({'error': 'Unauthorized'}), 403
...
def accept_booking(booking_id):
    ...
    try:
        ...
        supabase.table("bookings") \
            .update({"status": "Booked", "updated_at": datetime.utcnow().isoformat()}) \
            .eq("id", booking_id) \
            .execute()
